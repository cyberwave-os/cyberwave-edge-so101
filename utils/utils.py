...
# For motors 1-5 (body joints): min/max must be at least 20% away from center
# This ensures the robot was actually moved during calibration
BODY_JOINT_MIN_DISTANCE_FROM_CENTER_PERCENT = 0.20
# For motor 6 (gripper): range span must be at least 20% of full range (4095)
# Gripper uses 0-100 mapping, so it needs sufficient recorded range
GRIPPER_MIN_RANGE_SPAN_PERCENT = 0.20
...
def validate_calibration_ranges_sufficient(
    ...
    For body joints (motors 1-5): both min and max must be at least 20%
    away from the center position (2048). This ensures the joint was
    actually moved during calibration.

    For gripper (motor 6): the range span (max - min) must be at least 40%
    of the full range (4095). The gripper is mapped as 0-100, so it needs
    a wider recorded range.
    ...
    Returns:
        ...
        is_valid is True if all joints pass validation.
    """
    warnings: List[str] = []
    is_valid = True

    min_distance_from_center = SERVO_CENTER_POSITION * BODY_JOINT_MIN_DISTANCE_FROM_CENTER_PERCENT
    min_acceptable_max = SERVO_CENTER_POSITION + min_distance_from_center
    max_acceptable_min = SERVO_CENTER_POSITION - min_distance_from_center
    gripper_min_span = FULL_RANGE_MAX_RAW * GRIPPER_MIN_RANGE_SPAN_PERCENT
    ...
    for motor_name, calib_data in calibration.items():
        ...
        if motor_name == "gripper":
            # Gripper: check range span is at least 40% of full range
            range_span = range_max - range_min
            if range_span < gripper_min_span:
                is_valid = False
                warnings.append(
                    f"{motor_name}: Range span ({range_span:.0f}) is less than "
                    f"{GRIPPER_MIN_RANGE_SPAN_PERCENT*100:.0f}% of full range "
                    f"({gripper_min_span:.0f}). Move the gripper through its full "
                    f"open/close range during calibration."
                )
        else:
            # Body joints: check min is below threshold AND max is above threshold
            min_ok = range_min <= max_acceptable_min
            max_ok = range_max >= min_acceptable_max
            ...
            if not min_ok or not max_ok:
                ...
                if not min_ok:
                    issues.append(
                        f"min ({range_min:.0f}) should be <= {max_acceptable_min:.0f}"
                    )
                if not max_ok:
                    issues.append(
                        f"max ({range_max:.0f}) should be >= {min_acceptable_max:.0f}"
                    )
                warnings.append(
