...
def resolve_camera_sensor_id(twin: Any) -> str:
    """Return the stream sensor key a camera twin declares.

    The twin is the only source for this. Viewers subscribe by the sensor id in
    the asset schema, and the SDK derives the same key from
    ``capabilities.sensors[0].id``, so anything published under a different name
    is invisible to them — WebRTC still connects and the frame counter still
    advances, which makes the mismatch easy to miss. ``"default"`` mirrors the
    SDK's own fallback so both ends agree when a twin declares no sensors.
    """
    capabilities = getattr(twin, "capabilities", None) or {}
    sensors = capabilities.get("sensors") if isinstance(capabilities, dict) else None
...
def detect_voltage_rating(port: str, motor_id: int = 1, baudrate: int = 1000000) -> Optional[int]:
    ...
    for attempt in range(1, _DETECT_VOLTAGE_RETRIES + 1):
        ...
        try:
            ...
            # Same scservo_sdk timeout fix FeetechBus applies. The stock SDK
            # computes a packet timeout far too short to survive a USB/IP round
            # trip, so every Present_Voltage read comes back empty and a healthy
            # bus is reported as "no classifiable devices".
            try:
                from motors.feetech_bus import FeetechMotorsBus

                FeetechMotorsBus._patch_port_handler(port_handler)
            except Exception as patch_error:  # pragma: no cover - defensive
                # Loud on purpose: silently skipping the patch makes a healthy
                # bus look like absent hardware, which is very hard to diagnose.
                logger.warning(
                    "Could not patch scservo packet timeout (%s); voltage reads "
                    "may time out over USB/IP",
                    patch_error,
                )
