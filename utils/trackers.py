...
from utils.console import summary_interval_seconds, tui_enabled
...
class StatusTracker:
    ...
    def __init__(self):
        ...
        # Frames not MQTT-published because of fixed-rate downsampling (e.g. 2 of 3 at 100Hz);
        # not related to ignoring edge/sourced joint commands.
    ...
    def update_camera_status(self, camera_name: str, detected: bool, started: bool = False) -> None:
        with self.lock:
    ...
    def increment_filtered(self) -> None:
        """Count publish skipped due to downsampling (fixed N:1), not command filtering."""
        with self.lock:
    ...
    def update_joint_temperatures(self, temperatures: Dict[str, float]) -> None:
        """Replace joint temperatures with latest sampled snapshot."""
        with self.lock:
            self.joint_temperatures = temperatures.copy()
    ...
    def set_twin_info(
        ...
        """Set robot/camera display info when only a single camera row exists."""
        with self.lock:
...
def _format_status_summary(
    status: Dict[str, Any],
    mode: Literal["teleoperate", "remoteoperate"],
) -> str:
    """Condense a status snapshot into one log line for headless runs.

    Cameras collapse to an "ok/total" count and temperatures to the hottest
    joint; the per-joint detail only makes sense in the live panel.
    """
    camera_states = status.get("camera_states", {})
    if status.get("camera_enabled") and camera_states:
        streaming = sum(
            1
            for state in camera_states.values()
            if state.get("detected") and state.get("started")
        )
        cameras = f"{streaming}/{len(camera_states)}"
    else:
        cameras = "none"

    temps = [t for t in status.get("joint_temperatures", {}).values() if t is not None]
    hottest = f"{max(temps):.0f}C" if temps else "n/a"

    if mode == "teleoperate":
        throughput = f"produced={status.get('messages_produced', 0)}"
    else:
        throughput = (
            f"received={status.get('messages_received', 0)} "
            f"processed={status.get('messages_processed', 0)}"
        )

    return (
        f"status[{mode}] mqtt={'up' if status.get('mqtt_connected') else 'DOWN'} "
        f"cameras={cameras} fps={status.get('fps', 0)} "
        f"cam_fps={status.get('camera_fps', 0)} {throughput} "
        f"skipped={status.get('messages_filtered', 0)} "
        f"err_motor={status.get('errors_motor', 0)} "
        f"err_mqtt={status.get('errors_mqtt', 0)} hottest={hottest}"
    )
...
def run_status_logging_thread(
    ...
    *,
    ...
    - Motor overheating (>55°C warning, >65°C critical), unless mute_temperature_notifications is True
    - MQTT disconnected
    ...
    Args:
        ...
        mode: "teleoperate" (Prod/SkipPub stats, L+F temps) or "remoteoperate" (Recv/Proc/SkipPub, F temps)
        mute_temperature_notifications: If True, do not create motor overheating alerts (temps still logged).
    """
    status_tracker.fps = fps
    status_tracker.camera_fps = camera_fps
    status_interval = 1.0  # Update status at 1 fps

    # Only the rendering is gated; the alerting and telemetry below stay at
    # 1 Hz. Headless runs get a one-line summary instead (see utils.console).
    draw_tui = tui_enabled()
    ...
    try:
        while not stop_event.is_set():
            ...
            if robot is not None:
                ...
                if not mute_temperature_notifications:
                ...
                if status.get("errors_motor", 0) >= 10:
                    if create_motor_error_alert(robot, status["errors_motor"], threshold=10):
                        status_tracker.reset_errors_motor()
                if status.get("errors_mqtt", 0) >= 100:
                    if create_mqtt_error_alert(robot, status["errors_mqtt"], threshold=100):
                        status_tracker.reset_errors_mqtt()

                if mqtt_client is not None:
                    from utils.motor_telemetry import publish_robot_motor_status
            ...
            if not draw_tui:
                ...
                if now - last_summary_at >= summary_interval:
                    last_summary_at = now
                    logger.info(_format_status_summary(status, mode))
            ...
            if status["camera_enabled"]:
                ...
                for info in camera_infos:
                    ...
                    webrtc_icon = (
                        "🟢"
                        if webrtc == "streaming"
                        else ("🟡" if webrtc == "connecting" else "🔴")
                    )
                    lines.append(
            ...
            if mode == "teleoperate":
                stats = (
                    ...
                    f"Prod:{status['messages_produced']} "
                    f"SkipPub:{status['messages_filtered']} "
                ...
                stats = (
                    ...
                    f"SkipPub:{status['messages_filtered']} ErrM:{err_motor} ErrQ:{err_mqtt}"
                )
            ...
            if joint_states:
                ...
                for joint_index in sorted(joint_states.keys()):
                    ...
                    lines.append(
                        _format_motor_line(joint_index, joint_name, position, mode, joint_temps)[
                            :70
                        ].ljust(70)
                    )
            elif index_to_name:
                ...
                for joint_index in sorted(index_to_name.keys()):
                    ...
                    lines.append(
                        _format_motor_line(joint_index, joint_name, None, mode, joint_temps)[
                            :70
                        ].ljust(70)
                    )
            else:
