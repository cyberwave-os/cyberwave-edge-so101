...
EXCLUDED_CAMERA_NAMES = frozenset(
    {"pispbe", "rpivid", "rpi-hevc-dec", "bcm2835-codec-decode", "bcm2835-isp"}
)
...
def _probe_cv2_camera(device_path: str) -> tuple[bool, str]:
    ...
    Local ``/dev/video*`` nodes are opened with an explicit ``CAP_V4L2``. Letting
    OpenCV choose sends a string path to FFmpeg's libavdevice V4L2 demuxer first,
    which spams ``ioctl(VIDIOC_QBUF): Bad file descriptor`` on nodes that are not
    capture devices and probes a backend the SDK never streams through.
    """
    try:
    ...
    try:
        ...
        if device_path.startswith("/dev/video") and v4l2_backend is not None:
            cap = cv2.VideoCapture(device_path, v4l2_backend)
        else:
