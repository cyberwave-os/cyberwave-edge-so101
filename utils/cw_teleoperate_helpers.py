...
from utils.utils import normalized_to_radians, resolve_camera_sensor_id
...
def publish_initial_observations(
    ...
    motor_id_to_schema_joint: Dict[int, str],
    fps: int = 100,
    camera_twins: Optional[list] = None,
) -> None:
    """
    ...
    When *camera_twins* is provided, includes a ``camera_participants`` manifest
    listing each camera twin UUID and sensor name so the backend knows about all
    session participants before camera recordings are generated.
    """
    ...
    if observations:
        ...
        if camera_twins:
            ...
            for item in camera_twins:
                twin = item[0] if isinstance(item, tuple) else item
                cam_uuid = str(getattr(twin, "uuid", twin))
                participants.append(
                    {"twin_uuid": cam_uuid, "sensor_id": resolve_camera_sensor_id(twin)}
                )
            metadata["camera_participants"] = participants
