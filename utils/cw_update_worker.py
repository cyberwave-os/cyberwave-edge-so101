...
# Type alias for the queue item: (source_type, observation, timestamp, camera_frame_counters)
QueueItem = tuple[Optional[str], Dict[str, float], float, Optional[Dict[str, Dict[str, Any]]]]
...
def cyberwave_update_worker(
    ...
    Args:
        action_queue: Queue containing (source_type, observation, timestamp, camera_frame_counters) tuples
                     where observation is a dict with joint positions and camera_frame_counters is
                     an optional dict mapping track_id to {frame_count, sensor_id}
        ...
        try:
            ...
            while True:
                try:
                    ...
                    item = action_queue.get(timeout=min(remaining_time, 0.001))
                    # Support both old 3-tuple and new 4-tuple format for backward compat
                    if len(item) == 4:
                        source_type, observation, timestamp, camera_frame_counters = item
                    else:
                        source_type, observation, timestamp = item
                        camera_frame_counters = None
                    # Keep latest update per source_type
                    batch_by_source[source_type or "edge"] = (
                        observation,
                        timestamp,
                        source_type,
            ...
            for _source_key, (
                ...
                try:
                    ...
                    for joint_key, normalized_pos in observation.items():
                        ...
                        # Fall back to the canonical schema name "_N" (never a
                        # bare "N"): the backend no longer normalizes bare-numeric
                        # joint names, so a bare key would fail to resolve.
                        schema_joint = (
                            motor_id_to_schema_joint.get(joint_index, f"_{joint_index}")
                            if motor_id_to_schema_joint
                            else f"_{joint_index}"
                        )

                        positions[schema_joint] = position
                        joint_states[f"_{joint_index}"] = position

                    # SO101 only reads positions; do not fabricate velocity/effort.
...
# Type alias for camera frame snapshot getter
CameraFrameSnapshotGetter = Callable[[], Dict[str, Dict[str, Any]]]
...
def process_cyberwave_updates(
    ...
    camera_frame_snapshot_getter: Optional[CameraFrameSnapshotGetter] = None,
) -> int:
    """
    ...
    Args:
        ...
        camera_frame_snapshot_getter: Optional callable that returns a snapshot of camera
            frame counters at the moment of enqueueing. The snapshot is a dict mapping
            track_id to {"frame_count": int, "sensor_id": str}.
    Returns:
    ...
    if frame_count % PUBLISH_EVERY_N_FRAMES != 0:
        if status_tracker:
            # Not "filtered" commands—fixed-rate publish: skip this 100Hz sample for MQTT.
            status_tracker.increment_filtered()
        return 0

    # Capture camera frame snapshot at enqueue time (not at worker flush time)
    camera_frame_counters: Optional[Dict[str, Dict[str, Any]]] = None
    if camera_frame_snapshot_getter is not None:
        try:
            camera_frame_counters = camera_frame_snapshot_getter()
            # Only include if we have actual camera data
            if not camera_frame_counters:
                camera_frame_counters = None
        except Exception:
            camera_frame_counters = None

    try:
        action_queue.put_nowait((source_type, action, timestamp, camera_frame_counters))
        return len(action)
    except queue.Full:
        if status_tracker:
            status_tracker.increment_errors_mqtt()
