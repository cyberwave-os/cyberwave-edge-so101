...
def create_high_load_alert(
    twin: Twin,
    high_loads: Dict[str, float],
    threshold: float,
) -> bool:
    """
    Create an alert when motor load exceeds safety threshold.

    This is a critical safety alert - torque will be disabled.

    Args:
        twin: Twin instance with alerts
        high_loads: Dictionary of motor_name -> load value for motors exceeding threshold
        threshold: The load threshold that was exceeded

    Returns:
        True if alert was created, False if throttled
    """
    alert_key = "high_motor_load"
    if not _should_create_alert(alert_key, _MOTOR_ALERT_THROTTLE):
        return False

    motors_info = ", ".join(f"{name}={load:.1f}" for name, load in high_loads.items())
    description = (
        f"Motor load exceeded safety threshold ({threshold}). "
        f"Affected motors: {motors_info}. "
        "Torque has been disabled for safety. "
        "Check for obstructions or collisions before restarting."
    )

    try:
        twin.alerts.create(
            name="High motor load - torque disabled",
            description=description,
            alert_type="high_motor_load",
            severity="critical",
            source_type="edge",
            metadata={
                "high_loads": high_loads,
                "threshold": threshold,
                "action_taken": "torque_disabled",
            },
        )
        logger.info(f"Created high motor load alert: {motors_info}")
        return True
    except Exception as e:
        _log_alert_failure("high_motor_load", e)
        return False


_COLLISION_ALERT_THROTTLE = 5.0  # 5 seconds between collision alerts


def create_collision_alert(
    twin: Twin,
    collision_type: str,
    message: str,
    *,
    ...
    """
    Create an alert when a collision is detected (floor or self-collision).

    Args:
        twin: Twin instance with alerts
        collision_type: Type of collision ("floor" or "self")
        message: Detailed collision message from kinematics validation
        joint_angles: Optional dict of joint angles at collision time

    Returns:
        True if alert was created, False if throttled
    """
    alert_key = f"collision_{collision_type}"
    if not _should_create_alert(alert_key, _COLLISION_ALERT_THROTTLE):
        return False

    if collision_type == "floor":
        name = "Floor collision detected"
        severity = "warning"
    else:
        name = "Self-collision detected"
        severity = "warning"

    description = (
        f"{message}. "
        "The commanded pose was rejected for safety. "
        "Adjust the target position to avoid collision."
    )

    metadata: Dict[str, object] = {"collision_type": collision_type}
    if joint_angles:
    ...
    try:
        ...
        logger.info(f"Created collision alert: {collision_type} - {message}")
        return True
    except Exception as e:
        _log_alert_failure("collision_detected", e)
        return False
