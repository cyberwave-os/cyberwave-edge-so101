...
### macOS Docker Desktop: USB Device Passthrough

Docker Desktop on macOS runs containers inside a Linux VM that cannot directly access host USB devices. The SO-101 driver uses **USB/IP** to bridge this gap for both serial controllers and cameras.
...
6. The edge core starts driver containers with `--pid=host`
7. The container entrypoint uses `nsenter` + `usbip attach` to auto-discover and attach all USB devices
8. Serial controllers appear as `/dev/ttyACM*`, cameras as `/dev/video*`, RealSense devices via `libusb`

> **Note:** Auto-discovery attaches **all** USB devices exported by the host USB/IP server. The host operator controls which devices are exported (the `jiegec/usbip` server exports everything connected). If you need selective attachment, set `CYBERWAVE_USBIP_BUSID` to a specific bus ID in the container environment.
...
If camera benchmarks show poor frame rates, use the MJPEG stream fallback:
