...
Examples:
  ...
  python calibrate.py --type leader --port /dev/tty.usbmodem123 --id blue \\
    ...
    if twin_uuid and token:
        try:
            ...
            if alert_uuid:
                ...
                # Full snapshot so partial alert GETs don't drop guided-flow fields (e.g. buttons).
                _alert_metadata_snapshot = dict(_alert.metadata or {})

                def _update_alert_metadata(calibration_updates: dict) -> None:
                    nonlocal _calibration_meta, _alert_metadata_snapshot
                    try:
                        alert = _robot.alerts.get(alert_uuid)
                        if alert is None:
                            return
                        fresh = dict(alert.metadata or {})
                        server_meta = {**_alert_metadata_snapshot, **fresh}
                        ...
                        _alert_metadata_snapshot = server_meta
                    except Exception as e:
    ...
    def _on_state_change(state: str) -> None:
        """Log state changes but do NOT update alert metadata.

        State transitions are managed by main.py which creates new alerts
        for each calibration step (Step 1/2, Step 2/2). If we updated state
        here, we'd cause race conditions where Step 1/2 alert shows
        "joint_calibration_waiting" state (triggering "Complete" button)
        before main.py creates the Step 2/2 alert.
        """
        logger.debug("Calibration state change: %s (not updating alert)", state)

    def _on_joint_progress(
        current: dict, range_mins: dict, range_maxes: dict
    ) -> None:
        """Update joint positions in alert metadata (joints dict only)."""
    ...
    try:
        ...
        # and can show the Next / Complete buttons (MQTT fallback).
