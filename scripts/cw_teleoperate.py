...
from utils.cw_update_worker import (
...
def teleop_loop(
    ...
    Args:
        leader: Leader arm for reading observations
        follower: Follower arm for sending actions
        action_queue: Queue for Cyberwave updates
        stop_event: Event to signal loop exit
        time_reference: Shared time reference for synchronization
        status_tracker: Optional status tracker for statistics
        control_rate_hz: Control loop frequency (default: 100Hz)
        camera_manager: Optional camera stream manager for accessing frame counters.
            When provided, each joint update includes a snapshot of camera frame counters
            at the moment of sampling, enabling robot-camera synchronization.
    """
    total_update_count = 0
    control_frame_time = 1.0 / control_rate_hz

    # Create snapshot getter callable for process_cyberwave_updates
    snapshot_getter = (
        camera_manager.get_frame_counters
    ...
    try:
        while not stop_event.is_set():
            ...
            # Send both leader and follower observations to Cyberwave as aggregated messages
            if leader_obs:
                count = process_cyberwave_updates(
                    ...
                    camera_frame_snapshot_getter=snapshot_getter,
                )
...
def teleoperate(
    ...
    Args:
        ...
        setup_alert_uuid: If set, resolve this alert once streaming/workers are ready (edge-core).
    """
    ...
    try:
        ...
        # fps reflects actual MQTT publish rate after downsampling
        publish_fps = CONTROL_RATE_HZ // PUBLISH_EVERY_N_FRAMES
        if not telemetry_started:
            publish_initial_observations(
                ...
                fps=publish_fps,
                camera_twins=camera_twins if camera_twins else None,
            )
        ...
        if camera_twins and cyberwave_client is not None:
            ...
            for item in enriched_twins:
                twin = item[0] if isinstance(item, tuple) else item
                camera_infos.append(
                    {"uuid": str(twin.uuid), "name": resolve_camera_sensor_id(twin)}
                )
...
def main():
    ...
    # Omit key or set null for None (motor Vmax/Amax handle motion; no software stepping).
    _mrt = setup_config.get("max_relative_target")
    max_relative_target = float(_mrt) if _mrt is not None else None
    ...
    # Resolve twin UUID and ports: CLI/env > setup.json
    effective_twin_uuid = (
        args.twin_uuid
