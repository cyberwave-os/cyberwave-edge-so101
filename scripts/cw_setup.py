...
def so101_camera_name_override(camera_type: str, index: int = 0) -> str | None:
    """Stream sensor key override for RealSense on SO101 (matches backend pairing).

    Each camera gets a unique name so multi-camera setups register distinct
    entries in the SDK's CameraStreamManager registry.  The first camera
    keeps ``"camera"`` for backward compatibility with single-camera setups.
    """
    if "realsense" in str(camera_type).lower():
        return "camera" if index == 0 else f"camera_{index}"
    return None
...
def materialize_camera_entries_for_cli(client: Any, cameras: list[CameraConfig]) -> list[dict[str, Any]]:
    ...
    for i, c in enumerate(cameras):
        ...
        name = so101_camera_name_override(c.camera_type, index=i)
        if name is not None:
