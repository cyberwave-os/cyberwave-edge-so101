...
# NOTE: do not reintroduce a stream sensor key override here.
#
# The SDK derives the WebRTC/MQTT sensor key from the twin's
# ``capabilities.sensors[0].id``, and that is the key viewers subscribe to.
# A previous ``so101_camera_name_override`` forced ``"camera"`` for RealSense,
# which matched only while the D455 catalog asset shipped an empty ``sensors``
# array. Once the asset gained ``color_camera`` / ``depth_camera`` the edge kept
# publishing under ``camera``, so WebRTC connected and the frame counter
# advanced while the viewer stayed black. cv2 cameras never had the override,
# which is why they kept working. The registry is keyed by ``twin_uuid``, so
# distinct names are not needed for multi-camera setups.
...
def materialize_camera_entries_for_cli(
...
def save_setup_config(config: dict, path: Path | None = None) -> Path:
    """Atomically save setup config so readers never observe a partial JSON document."""
...
def _handle_invalid_setup_config(path: Path, reason: object, *, repair: bool) -> dict:
    if not repair:
        logger.error("Ignoring invalid SO101 setup config at %s: %s", path, reason)
        return {}

    logger.warning("Removing invalid SO101 setup config at %s: %s", path, reason)
    try:
        path.unlink()
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.warning("Could not remove invalid SO101 setup config at %s: %s", path, exc)
    return {}


def load_setup_config(path: Path | None = None, *, repair: bool = False) -> dict:
    """Load setup config.

    With ``repair=True``, remove an unparseable or non-object file so the
    startup bootstrap can regenerate it. Otherwise, report invalid content and
    return an empty config without modifying a user-supplied file.
    """
    path = path or get_setup_config_path()
    if not path.exists():
        return {}
    try:
        ...
        return _handle_invalid_setup_config(path, exc, repair=repair)

    if not isinstance(config, dict):
        return _handle_invalid_setup_config(
            path,
...
Examples:
  ...
  so101-setup --twin-uuid <so101-uuid> \\
    ...
    config["mute_temperature_notifications"] = getattr(
        args, "mute_temperature_notifications", False
