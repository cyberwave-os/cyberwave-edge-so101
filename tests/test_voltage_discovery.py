...
class TestScservoPacketTimeoutPatch:
    """USB/IP adds round-trip latency that the stock scservo_sdk timeout cannot
    absorb, so every Present_Voltage read returns nothing and autodiscovery
    reports "no classifiable devices" on a perfectly healthy bus.
    ``FeetechBus`` already patches ``setPacketTimeout``; autodiscovery opens its
    own PortHandler and must apply the same workaround.
    """

    def test_detect_voltage_rating_patches_packet_timeout(self, fake_scservo_module) -> None:
        handlers: list[object] = []

        class _FakePortHandler:
            def __init__(self, port: str) -> None:
                self.port = port
                self.tx_time_per_byte = 1.0
                self.packet_start_time = 0.0
                self.packet_timeout = None
            ...
            def setPacketTimeout(self, _packet_length: int) -> None:
                # Stock (buggy) SDK behaviour: a timeout far too short to survive
                # a USB/IP round trip.
        ...
        class _FakePacketHandler:
            ...
            def read1ByteTxRx(self, _ph: object, _mid: int, _addr: int):
                return (54, 0, None)
        ...
        with patch.object(uu, "time") as m_time:
            m_time.sleep = lambda _s: None
            uu.detect_voltage_rating("/dev/ttyFAKE")

        assert handlers, "expected a PortHandler to be constructed"
        ph = handlers[0]
        ph.setPacketTimeout(10)
        assert ph.packet_timeout is not None
        assert ph.packet_timeout >= 50, (
            "autodiscovery must apply the scservo_sdk packet-timeout patch; "
            f"got {ph.packet_timeout}"
        )
