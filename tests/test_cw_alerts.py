...
class TestCreateCollisionAlert:
    """Tests for collision_detected alert."""

    def test_creates_floor_collision_alert(self):
        """Floor collision alert should have correct type and severity."""
        _reset_alert_state()
        twin = _fake_twin()

        created = create_collision_alert(
            twin,
            collision_type="floor",
            message="Floor collision: gripper too close to floor (dist=0.005m < 0.01m)",
        )

        assert created is True
        assert len(twin.alerts.calls) == 1
        alert = twin.alerts.calls[0]
        assert alert["alert_type"] == "collision_detected"
        assert alert["name"] == "Floor collision detected"
        assert alert["severity"] == "warning"
        assert alert["source_type"] == "edge"
        assert "floor" in alert["metadata"]["collision_type"]

    def test_creates_self_collision_alert(self):
        """Self-collision alert should have correct type and metadata."""
        _reset_alert_state()
        twin = _fake_twin()
        ...
        created = create_collision_alert(
            twin,
            collision_type="self",
            message="Self-collision: upper_arm ↔ wrist too close",
        ...
        alert = twin.alerts.calls[0]
        assert alert["alert_type"] == "collision_detected"
        assert alert["name"] == "Self-collision detected"
        assert alert["severity"] == "warning"
        assert alert["metadata"]["collision_type"] == "self"
        assert alert["metadata"]["joint_angles"] == joint_angles

    def test_collision_alert_is_throttled(self, monkeypatch):
        """Collision alerts should be throttled to prevent spam."""
        _reset_alert_state()
        twin = _fake_twin()
        now = [1000.0]
        monkeypatch.setattr(cw_alerts.time, "time", lambda: now[0])
        ...
        created1 = create_collision_alert(twin, "floor", "Floor collision detected")
        assert created1 is True

        # Second alert within throttle window should be blocked
        now[0] += 2.0  # Only 2 seconds later
        created2 = create_collision_alert(twin, "floor", "Floor collision detected again")
        assert created2 is False

        # After throttle expires, alert should be created
        now[0] += 10.0  # 5 second throttle
        created3 = create_collision_alert(twin, "floor", "Floor collision detected")
        assert created3 is True

        assert len(twin.alerts.calls) == 2

    def test_different_collision_types_throttled_separately(self):
        """Floor and self-collision should have separate throttle timers."""
        _reset_alert_state()
        twin = _fake_twin()

        # Both should be created as they're different collision types
        created_floor = create_collision_alert(twin, "floor", "Floor collision")
        created_self = create_collision_alert(twin, "self", "Self collision")

        assert created_floor is True
