...
class TestChildTwinUuidsFromEdgeCore:
    ...
    def test_parse_child_camera_stream_urls(self, monkeypatch):
        monkeypatch.setenv(
            "CYBERWAVE_CHILD_CAMERA_STREAM_URLS",
            json.dumps(
                {
                    "u1": "http://host.docker.internal:8091",
                    "u2": "https://camera.example/stream",
                    "invalid": "/dev/video0",
                    "not-a-string": 8093,
                }
            ),
        ...
        assert main_module._parse_child_camera_stream_urls_from_env() == {
            "u1": "http://host.docker.internal:8091",
            "u2": "https://camera.example/stream",
        }
...
class TestSo101PortRefresh:
    ...
    def test_apply_ports_keeps_setup_when_discovery_returns_none_for_role(
        self, monkeypatch, tmp_path
    ):
        """If voltage scan cannot classify a role, we do not overwrite with None — keep file value.

        The persisted path is a real file here: an unclassifiable role whose
        device still exists is kept (motors may simply be unpowered), while a
        path that has vanished is pruned by
        ``test_persisted_port_that_no_longer_exists_is_dropped``.
        """
        monkeypatch.delenv("CYBERWAVE_METADATA_LEADER_PORT", raising=False)
        monkeypatch.delenv("CYBERWAVE_METADATA_FOLLOWER_PORT", raising=False)
        live_leader = tmp_path / "ttyACM0"
        live_leader.write_text("")
        existing = {
            "leader_port": str(live_leader),
            "follower_port": "/dev/ttyACM1",
        }
        with patch(
            "utils.utils.discover_so101_ports_by_voltage",
        ...
        assert existing["leader_port"] == str(live_leader)
    ...
    def test_start_freedrive_forces_follower_calibration(self):
        """An uncalibrated follower blocks a direct freedrive command: freedrive
        reports measured angles as the twin's pose, so it must not stream raw
        counts read as radians. Triggers follower calibration instead."""
        mock_client = MagicMock()
        so101 = So101Config.from_dict(
            {
                "twin_uuid": "t-1",
        ...
        with (
            patch.object(main_module, "_refresh_so101_ports_in_setup"),
            patch.object(main_module, "_is_follower_calibrated", return_value=False),
            patch(
                "scripts.cw_setup.load_so101_config_for_robot_twin",
                return_value=so101,
            ),
            ...
            main_module.start_freedrive(mock_client, "t-1")

        trigger.assert_called_once()
        # Follower is the default device_type (same as start_remoteoperate).
        assert trigger.call_args.kwargs.get("device_type", "follower") == "follower"
        # Must resume freedrive (not teleop/remoteop) once calibration completes.
        assert trigger.call_args.kwargs["recovery_command"] == "freedrive"
        thread.assert_not_called()

    def test_start_freedrive_forces_present_leader_calibration(self):
        """A present-but-uncalibrated leader blocks freedrive too, even though
        freedrive only reads the follower — every present arm is calibrated up
        front, matching teleoperate/remoteoperate."""
        mock_client = MagicMock()
        so101 = So101Config.from_dict(
            {
                "twin_uuid": "t-1",
    ...
    def test_start_freedrive_runs_when_no_leader_present(self):
        """A calibrated follower-only setup is not blocked on leader calibration."""
        mock_client = MagicMock()
        so101 = So101Config.from_dict(
            {
                "twin_uuid": "t-1",
    ...
    def _run_freedrive_thread_body(self, connect_ok=True):
        """Run start_freedrive's operation body inline, with the session mocked.

        threading.Thread is a MagicMock in these tests, so the body never runs on
        its own; the driver-side contract (build device, register, gate on connect)
        only shows up when it is invoked directly.
        """
        so101 = So101Config.from_dict(
            {"twin_uuid": "t-1", "leader_port": "", "follower_port": "/dev/b"}
        )
        session = MagicMock()
        # Record what was registered at each step, so the ordering is checkable.
        seen: list = []

        def fake_connect():
            seen.append(("connect", main_module._current_follower))
            return connect_ok

        session.connect.side_effect = fake_connect

        def fake_freedrive(**kwargs):
            seen.append(("loop", main_module._current_follower))
        ...
        with (
            ...
            patch.object(main_module, "_is_follower_calibrated", return_value=True),
            patch.object(main_module, "_is_leader_calibrated", return_value=True),
            patch("scripts.cw_setup.load_so101_config_for_robot_twin", return_value=so101),
            patch(
                "scripts.cw_setup.materialize_camera_entries_for_edge_operation",
                return_value=[],
            ),
            patch("utils.cw_freedrive.FreedriveSession", return_value=session),
            patch("utils.cw_freedrive.freedrive", new=fake_freedrive),
            patch.object(main_module, "create_robot_setup_alert", return_value="alert-1"),
            patch.object(main_module, "_resolve_alert_by_uuid") as resolve,
            patch.object(main_module, "_create_error_alert") as error_alert,
            patch.object(main_module.threading, "Thread", new=MagicMock()) as thread,
        ):
            try:
                main_module.start_freedrive(MagicMock(), "t-1")
                thread.call_args.kwargs["target"]()
                after = main_module._current_follower
            finally:
                main_module._current_follower = None
                main_module._current_thread = None

        return session, seen, after, resolve, error_alert

    def test_start_freedrive_registers_the_session_as_current_follower(self):
        """_stop_current_operation force-disconnects _current_follower when the
        operation thread will not exit. Freedrive holds the port through a session
        rather than an SO101Follower (that path enables torque and would make the
        arm rigid), so the session is what has to be registered -- and it has to be
        registered before connect(), since from that call onwards it owns the port.
        """
        session, seen, after, _, error_alert = self._run_freedrive_thread_body()

        # Registered before the port was opened, and still registered in the loop.
        assert seen == [("connect", session), ("loop", session)]
        # Cleared afterwards, so a dead session is never force-disconnected.
        assert after is None
        session.disconnect.assert_called_once()
        error_alert.assert_not_called()

    def test_start_freedrive_alerts_when_the_port_will_not_open(self):
        """The gateway remoteoperate has around follower.connect(): a port that
        will not open is reported, not retried silently behind the backoff."""
        session, seen, after, resolve, error_alert = self._run_freedrive_thread_body(
            connect_ok=False
        )

        assert seen == [("connect", session)], "the loop must not start when the port did not open"
        assert error_alert.call_args.kwargs["error_type"] == "follower_connection_failed"
        resolve.assert_any_call(ANY, "t-1", "alert-1")
        # Still released and cleared, so the next operation inherits nothing.
        session.disconnect.assert_called_once()
        assert after is None
...
class TestResolveAssignedControllerType:
    ...
    def test_recovery_command_freedrive_is_freedrive(self):
        client = MagicMock()
        with patch.object(
            main_module, "_resolve_assigned_controller_type", return_value="freedrive"
        ):
            assert (
                main_module._resolve_assigned_controller_recovery_command(client, "t-1")
                == "freedrive"
...
class TestOpForControllerType:
    """The single mapping from controller policy type -> driver operation."""

    def test_localop_maps_to_teleoperate(self):
        assert main_module._op_for_controller_type("localop") == "teleoperate"

    def test_freedrive_maps_to_freedrive(self):
        assert main_module._op_for_controller_type("freedrive") == "freedrive"

    def test_unknown_types_default_to_remoteoperate(self):
        for ct in ("teleop", "code", "mlmodel", "vla", ""):
            assert main_module._op_for_controller_type(ct) == "remoteoperate"

    def test_no_controller_maps_to_none(self):
        assert main_module._op_for_controller_type(None) is None

    def test_every_mapped_op_is_a_recovery_command(self):
        """A pinned op must survive calibration; freedrive is no exception."""
        for ct in ("localop", "freedrive", "teleop"):
            assert main_module._op_for_controller_type(ct) in main_module.RECOVERY_COMMANDS
...
def _alive_thread_factory() -> MagicMock:
    """A ``threading.Thread`` stand-in whose instances report ``is_alive() is
    True``, so ``_is_control_operation_running`` sees a real boolean."""
    factory = MagicMock()
    factory.return_value.is_alive.return_value = True
    return factory


class TestActuatingOperationPredicate:
    """The watchdog only guards operations that can move or energize the robot,
    so freedrive must read as running-but-not-actuating."""

    def setup_method(self):
        # Earlier tests start operations with threading.Thread mocked, leaving a
        # live-looking _current_thread behind; reset so these read real state.
        main_module._current_thread = None
        main_module._current_operation_actuates = False

    teardown_method = setup_method

    def test_idle_is_not_actuating(self):
    ...
    def test_actuating_control_operation_counts(self):
        """teleoperate/remoteoperate write to the follower -> guarded."""
        main_module._current_operation_actuates = True
        with (
            patch.object(main_module, "_is_control_operation_running", return_value=True),
            patch.object(main_module, "_is_calibration_running", return_value=False),
        ):
            assert main_module._is_actuating_operation_running() is True

    def test_read_only_control_operation_does_not_count(self):
        """A running freedrive is an operation, but not one the watchdog guards."""
        main_module._current_operation_actuates = False
        with (
            patch.object(main_module, "_is_control_operation_running", return_value=True),
            patch.object(main_module, "_is_calibration_running", return_value=False),
    ...
    def test_calibration_counts_even_without_control_thread(self):
        """A calibration subprocess holds torque -> guarded."""
        with (
            patch.object(main_module, "_is_control_operation_running", return_value=False),
            patch.object(main_module, "_is_calibration_running", return_value=True),
        ):
    ...
    def test_start_freedrive_marks_operation_non_actuating(self):
        """The flag is wired by the entry point, not inferred from thread names."""
        so101 = So101Config.from_dict(
            {"twin_uuid": "t-1", "follower_port": "/dev/b"}
        )
        with (
            patch.object(main_module, "_refresh_so101_ports_in_setup"),
            patch.object(main_module, "_stop_idle_camera_streaming"),
            # Freedrive is calibration-gated like teleoperate/remoteoperate, so the
            # checks must be pinned here — otherwise they read the host's real
            # calibration dir and this test passes or fails per developer machine.
            patch.object(main_module, "_is_follower_calibrated", return_value=True),
            patch.object(main_module, "_is_leader_calibrated", return_value=True),
            patch(
                "scripts.cw_setup.load_so101_config_for_robot_twin",
            ...
            patch.object(main_module.threading, "Thread", new=_alive_thread_factory()),
            patch.object(main_module, "_is_calibration_running", return_value=False),
        ):
            main_module.start_freedrive(MagicMock(), "t-1")

            assert main_module._current_operation_actuates is False
            # It is a running operation — just not one the watchdog guards.
            assert main_module._is_control_operation_running() is True
            assert main_module._is_actuating_operation_running() is False

    def test_start_teleoperate_marks_operation_actuating(self):
        so101 = So101Config.from_dict(
            {"twin_uuid": "t-1", "leader_port": "/dev/a", "follower_port": "/dev/b"}
        )
        with (
            patch.object(main_module, "_refresh_so101_ports_in_setup"),
            patch.object(main_module, "_stop_idle_camera_streaming"),
            ...
            main_module.start_teleoperate(MagicMock(), "t-1")

            assert main_module._current_operation_actuates is True
            assert main_module._is_actuating_operation_running() is True
...
class TestEvaluateConnectionWatchdog:
    ...
    def test_connected_clears_timer_and_never_trips(self):
        ...
        since, trip = main_module._evaluate_connection_watchdog(
            connected=True,
            actuating_operation_running=True,
    ...
    def test_disconnected_while_non_actuating_never_trips(self):
        """An outage with no actuating operation (idle, or freedrive only) starts
        the timer but never trips."""
        since, trip = main_module._evaluate_connection_watchdog(
            connected=False,
            actuating_operation_running=False,
...
class TestConnectionWatchdogLoop:
    ...
    def test_trips_on_sustained_outage_during_operation(self):
        ...
        with (
            patch.object(main_module, "_is_actuating_operation_running", return_value=True),
            patch.object(
    ...
    def test_trips_during_calibration_too(self):
        """Calibration counts as an actuating operation for the watchdog: a
        follower calibration is energized from connect() to disconnect(), and it
        can only be advanced/cancelled over MQTT."""
        client = MagicMock()
        client.mqtt.connected = False
        on_trip = MagicMock()
    ...
    def test_does_not_trip_during_freedrive(self):
        """A read-only operation is not guarded: an outage that would trip a
        teleop leaves a running freedrive alone, so the driver keeps streaming
        and stays up for the SDK to reconnect."""
        client = MagicMock()
        client.mqtt.connected = False
        on_trip = MagicMock()
        main_module._current_operation_actuates = False
        ...
        try:
            with (
                ...
                patch.object(main_module.time, "monotonic", return_value=1000.0),
            ):
                main_module._connection_watchdog_loop(
                    client,
                    "twin-1",
...
class TestMjpegFallbackForRgbChildCameras:
    """macOS has no /dev/video* nodes, so the host publishes the camera as an
    MJPEG stream and edge-core passes the URL in CYBERWAVE_METADATA_VIDEO_DEVICE.
    Depth twins already fall back to it; plain RGB twins must too, otherwise a
    macOS SO101 silently runs with no camera at all.
    """

    @staticmethod
    def _rgb_twin(uuid: str) -> dict:
        return {"uuid": uuid, "metadata": {}, "asset": {"registry_id": "x/standard-cam"}}
    ...
    def _run(
        ...
        monkeypatch.setenv("CYBERWAVE_METADATA_VIDEO_DEVICE", url)
        if stream_urls is None:
            monkeypatch.delenv("CYBERWAVE_CHILD_CAMERA_STREAM_URLS", raising=False)
        else:
            monkeypatch.setenv(
                "CYBERWAVE_CHILD_CAMERA_STREAM_URLS",
                json.dumps(stream_urls),
            )
        with (
            patch.object(
                main_module,
                "_load_discovered_devices",
                return_value=([] if discovered is None else discovered, 0),
            ),
            patch.object(
                main_module,
                "_parse_child_camera_twin_uuids_from_env",
    ...
    def test_rgb_child_binds_to_the_mjpeg_stream(self, monkeypatch):
        cameras = self._run(monkeypatch, ["child-a"], [self._rgb_twin("child-a")])

        assert len(cameras) == 1
        assert cameras[0]["twin_uuid"] == "child-a"
        assert cameras[0]["video_device"] == "http://host.docker.internal:8091"
        assert cameras[0]["camera_type"] == "cv2"
        assert cameras[0]["enable_depth"] is False

    def test_one_stream_is_not_handed_to_two_twins(self, monkeypatch):
        """A single host stream can only serve one twin; the second must not
        silently duplicate it."""
        cameras = self._run(
            monkeypatch,
            ["child-a", "child-b"],
            [self._rgb_twin("child-a"), self._rgb_twin("child-b")],
        )

        assert len(cameras) == 1

    def test_three_children_bind_to_three_macos_streams(self, monkeypatch):
        child_uuids = ["child-a", "child-b", "child-c"]
        urls = {
            child_uuid: f"http://host.docker.internal:{8091 + index}"
            for index, child_uuid in enumerate(child_uuids)
        }

        cameras = self._run(
            monkeypatch,
            child_uuids,
            [self._rgb_twin(child_uuid) for child_uuid in child_uuids],
            stream_urls=urls,
        )

        assert {camera["twin_uuid"]: camera["video_device"] for camera in cameras} == urls

    def test_usbip_devices_and_macos_streams_can_fill_three_children(self, monkeypatch):
        child_uuids = ["child-a", "child-b", "child-c"]
        streamed_url = "http://host.docker.internal:8093"
        discovered = [
            {
                "card": "USB camera A",
                "primary_path": "/dev/video0",
        ...
        cameras = self._run(
            ...
            child_uuids,
            [self._rgb_twin(child_uuid) for child_uuid in child_uuids],
            stream_urls={"child-c": streamed_url},
            discovered=discovered,
        )

        assert [camera["video_device"] for camera in cameras] == [
            "/dev/video0",
            "/dev/video2",
            streamed_url,
        ]

    def test_no_stream_configured_still_yields_nothing(self, monkeypatch):
        """Without a stream URL there is genuinely no camera to bind."""
        cameras = self._run(monkeypatch, ["child-a"], [self._rgb_twin("child-a")], url="")
