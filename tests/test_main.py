...
class TestCameraStreamSensorOverride:
    ...
    def test_camera_name_override_realsense_uses_camera(self):
        """RealSense streams use 'camera' sensor key for backward compatibility."""
        assert main_module._camera_name_override("realsense") == "camera"
        assert main_module._camera_name_override("RealSense") == "camera"
    ...
    def test_build_idle_camera_twins_sets_camera_sensor_for_realsense(self):
        """Idle camera stream config includes camera_name=camera for RealSense."""
        client = MagicMock()
        ...
        assert overrides["camera_name"] == "camera"
