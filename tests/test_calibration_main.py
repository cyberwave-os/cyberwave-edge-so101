...
def mock_client():
    ...
    # Controller-type resolution reads the backend via client.twins.get_raw
    # (TwinSchema: controller_policy_uuid + metadata.controller_type). Default
    # to "no controller attached"; tests exercising a controller override
    # client.twins.get_raw.return_value via set_attached_controller().
    client.twins.get_raw.return_value = _raw_twin(uuid=None)
    return client


def _raw_twin(*, uuid, controller_type=None):
    """Build a mock TwinSchema as client.twins.get_raw returns it."""
    twin_data = MagicMock()
    twin_data.controller_policy_uuid = uuid
    twin_data.metadata = (
        {"controller_type": controller_type} if controller_type is not None else {}
    )
    return twin_data


def set_attached_controller(client, *, controller_type):
    """Make the mock backend report an attached controller of the given type."""
    client.twins.get_raw.return_value = _raw_twin(
        uuid="policy-1", controller_type=controller_type
    )
...
class TestResolveAssignedControllerRecoveryCommand:
    ...
    def test_reads_localop_controller_via_backend(self, mock_client):
        """A localop controller (from the get_raw metadata mirror) -> teleoperate."""
        import main as main_module

        set_attached_controller(mock_client, controller_type="localop")
    ...
    def test_reads_teleop_controller_via_backend(self, mock_client):
        """A non-localop (e.g. keyboard 'teleop') controller -> remoteoperate."""
        import main as main_module

        set_attached_controller(mock_client, controller_type="teleop")
        result = main_module._resolve_assigned_controller_recovery_command(
            mock_client, "twin-123"
        )
    ...
    def test_returns_none_when_no_controller_attached(self, mock_client):
        ...
        # Fixture default: get_raw reports controller_policy_uuid=None.
        result = main_module._resolve_assigned_controller_recovery_command(
            mock_client, "twin-123"
        )
    ...
    def test_attached_without_mirror_fetches_policy_type(self, mock_client):
        """Attached controller whose metadata mirror is absent -> fetch the policy."""
        import main as main_module

        mock_client.twins.get_raw.return_value = _raw_twin(uuid="policy-1")
        policy = MagicMock()
        policy.controller_type = "localop"
        mock_client.api.src_app_api_controller_policies_get_controller_policy.return_value = (
            policy
        )
        result = main_module._resolve_assigned_controller_recovery_command(
            mock_client, "twin-123"
    ...
    def test_backend_is_authoritative_over_stale_local_twin_json(self, mock_client):
        ...
        # Fixture default: get_raw reports no controller (authoritative).
        with patch.object(
    ...
    def test_falls_back_to_local_twin_json_when_backend_unreachable(self, mock_client):
        ...
        mock_client.twins.get_raw.side_effect = RuntimeError("backend offline")
...
class TestEvaluateAndDrive:
    ...
    def test_leader_missing_for_remoteop_triggers_leader_calibration(self, mock_client):
        """All missing calibrations are forced: a present leader must be calibrated
        before the controller is attached, even for a keyboard/remote controller."""
        trigger, teleop, remoteop = self._run(
    ...
    def test_leader_missing_but_absent_for_remoteop_runs_remoteoperate(self, mock_client):
        """A follower-only setup (no leader port) is not blocked on leader calibration."""
        import main as main_module

        cfg = {**self.CFG, "leader_port": ""}
        set_attached_controller(mock_client, controller_type="keyboard")
    ...
    def test_all_calibrated_resumes_teleoperate(self, mock_client):
        ...
        + controller attached → teleoperate is started instead of sitting idle."""
        set_attached_controller(mock_client, controller_type="localop")
        trigger, teleop, remoteop = self._run(
    ...
    def test_all_calibrated_detached_controller_stays_idle(self, mock_client):
        """Pinned op but controller still detached (re-attach window pending)
        → nothing starts; the controller-changed from the re-attach is the
        single path that starts the op (no take-initiative bounce)."""
        trigger, teleop, remoteop = self._run(
            mock_client, pinned="remoteoperate", follower_ok=True, leader_ok=True
        )
        trigger.assert_not_called()
...
class TestCheckStartupCalibration:
    ...
    def test_forces_leader_calibration_for_remoteoperate_controller(self, mock_client):
        """At startup a present-but-uncalibrated leader is forced to calibrate even
        for a remoteoperate controller — the controller is not attached early."""
        import main as main_module
        ...
        assert mock_trigger.call_args.kwargs["device_type"] == "leader"
