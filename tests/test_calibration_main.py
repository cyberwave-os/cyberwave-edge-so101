...
class TestCalibrationAlertPayload:
    ...
    def test_trigger_alert_includes_media_url(self, mock_client):
        """Calibration-needed alert should include guidance media and start button payload."""
        import main as main_module
        ...
        assert kwargs["metadata"]["buttons"][0]["label"] == "Start calibration"
        assert kwargs["metadata"]["buttons"][0]["payload"]["flow"] == "so101_calibration"
        assert kwargs["metadata"]["buttons"][0]["payload"]["action"] == "start"
