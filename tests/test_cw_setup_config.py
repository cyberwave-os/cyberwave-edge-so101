...
class TestSetupConfigPersistence:
    ...
    def test_load_removes_invalid_setup_and_returns_empty_config(self, tmp_path, caplog, contents):
        setup_path = tmp_path / "setup.json"
        setup_path.write_text(contents)

        assert cw_setup.load_setup_config(setup_path, repair=True) == {}
        assert not setup_path.exists()
        assert "Removing invalid SO101 setup config" in caplog.text

    def test_load_without_repair_keeps_invalid_file(self, tmp_path, caplog):
        setup_path = tmp_path / "setup.json"
        setup_path.write_text("{not-json")

        assert cw_setup.load_setup_config(setup_path) == {}
        assert setup_path.read_text() == "{not-json"
        assert "Ignoring invalid SO101 setup config" in caplog.text

    def test_save_failure_preserves_previous_setup(self, tmp_path):
        setup_path = tmp_path / "setup.json"
        original = {"twin_uuid": "robot-1", "cameras": []}
        setup_path.write_text(json.dumps(original))

        def fail_after_partial_write(config, handle, indent):
            handle.write("{")
            raise RuntimeError("simulated interrupted write")

        with patch.object(cw_setup.json, "dump", side_effect=fail_after_partial_write):
            with pytest.raises(RuntimeError, match="simulated interrupted write"):
                cw_setup.save_setup_config({"twin_uuid": "robot-2"}, setup_path)

        assert json.loads(setup_path.read_text()) == original
        assert list(tmp_path.glob(".setup.json.*.tmp")) == []

    def test_save_preserves_world_readable_mode_on_overwrite(self, tmp_path):
        setup_path = tmp_path / "setup.json"
        setup_path.write_text('{"twin_uuid": "robot-1"}')
        setup_path.chmod(0o644)

        cw_setup.save_setup_config({"twin_uuid": "robot-2"}, setup_path)

        assert stat.S_IMODE(setup_path.stat().st_mode) == 0o644

    def test_save_new_file_is_readable_by_other_users(self, tmp_path):
        setup_path = tmp_path / "setup.json"

        cw_setup.save_setup_config({"twin_uuid": "robot-1"}, setup_path)

        assert stat.S_IMODE(setup_path.stat().st_mode) & 0o044 == 0o044

    def test_ensure_setup_recreates_invalid_setup(self, tmp_path, monkeypatch):
        ...
        with patch("utils.utils.ensure_video_device_permissions"):
            with patch.object(main_module, "_apply_so101_ports_env_and_discovery"):
                with patch.object(main_module, "_discover_cameras_for_so101", return_value=[]):
                    main_module._ensure_setup("robot-1")

        assert json.loads(setup_path.read_text())["twin_uuid"] == "robot-1"
...
class TestMaxCamerasChildPath:
    def test_discover_child_path_stops_at_max_cameras(self):
        ...
        with patch.object(
            ...
            main_module, "_load_all_twin_jsons", return_value=twins
        ), patch.object(
            main_module, "_resolve_camera_device_for_twin", return_value=None
        ), patch.object(
            main_module, "_load_edge_fingerprint", return_value=None
        ):
