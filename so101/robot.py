...
from utils.errors import DeviceNotConnectedError, InsufficientCalibrationRangeError
from utils.cw_alerts import get_calibration_media_url
from utils.utils import validate_calibration_ranges_sufficient
...
class SO101Robot(abc.ABC):
    ...
    def get_motor_register_snapshot(
        self,
        ...
        *,
        ...
        """Read a register snapshot for all configured motors."""
        if not self.connected or self.bus is None:
            raise DeviceNotConnectedError(f"{self} is not connected")
        return self.bus.read_all_registers(
    ...
    def calibrate(
        ...
        Args:
            ...
            robot: Optional Cyberwave twin object. When provided, a "Calibration
                Needed" alert is created at each blocking step so the frontend can
                show a "Go Ahead" button. Idempotent — the backend deduplicates
                alerts with the same name/twin/description.
        """
        ...
        device_type = "leader" if "Leader" in type(self).__name__ else "follower"
        zero_position_alert = None
        if robot:
            try:
                zero_position_alert = robot.alerts.create(
                    name=f"Calibrate {self} - First step",
                    description=f"Move {self} to the middle of its range of motion and then click on `Go Ahead`",
                    media=get_calibration_media_url(device_type, "zero"),
                    severity="warning",
        ...
        if robot:
            # Frontend flow: edge main injects Enter when user clicks "Go Ahead".
            input()
        else:
        ...
        if robot:
            try:
                if zero_position_alert:
                    zero_position_alert.resolve()
                robot.alerts.create(
                    name=f"Calibrate {self} - Second step",
                    description=f"Move all joints of {self} through their entire ranges of motion. Click `Go Ahead` when done.",
                    media=get_calibration_media_url(device_type, "range"),
                    severity="warning",
                    alert_type="calibration_needed",
                )
            except Exception:
        ...
        if robot:
            print(
                ...
                "of motion.\nRecording positions. Click `Go Ahead` when done."
            )
        else:
            print(
                "Move all joints sequentially through their entire ranges "
        ...
        # Validate ranges before saving (5% error, 20% warning thresholds)
        calib_data = {
        ...
        if not is_valid:
            raise InsufficientCalibrationRangeError(warnings=warnings, severity=severity or "error")
